...
public class Clock extends View {
	...
	public Clock(Context context, AttributeSet attrs, int defStyle) {
		...
		vec = new Vector<MyTime>();

		this.setOnChoseListener(new OnChoseListener() {

			@Override
			public void onChose(int position) {
				Toast.makeText(Clock.this.getContext(),
						"Chose On: " + position, Toast.LENGTH_SHORT / 2).show();
			}
		});
	}
	...
	protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
		...
		this.setOnTouchListener(new OnTouchListener() {
			...
			public boolean onTouch(View v, MotionEvent event) {
				...
				int position;
				if (posX < Clock.this.getWidth() / 2) {
					if (posY < Clock.this.getHeight() / 2) {
						position = 0;
					} else {
						position = 2;
					}
	...
	public void addTimePoint(MyTime obj) {
		if (vec == null)
			vec = new Vector<MyTime>();
		vec.add(obj);
		mChanged = true;
	...
	public boolean removeTimePoint(MyTime obj) {
		if (vec == null)
			vec = new Vector<MyTime>();
		boolean ret = vec.remove(obj);
		invalidate();
		mChanged = true;
		return ret;
	}
